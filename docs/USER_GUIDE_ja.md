...
設定画面には言語、システム情報、Wi-Fi、バックアップと復元、OSC送信先、Key 1～12、Encoderの順に表示されます。システム欄では製品名、XIAOモデル、ファームウェアバージョン、IPアドレス、mDNS名を確認できます。変更後は画面に追従する保存ボタンで一括保存します。
...
プリセットは同種デバイスなら他のChainOSCデバイスと共有できます。[ChainOSC Device Presets](https://github.com/shimez/ChainOSC/tree/main/presets)では、VRChatなどで利用できるデバイスプリセットのサンプルを公開しています。ダウンロードしたJSONファイルをインポートして利用できます。
