# ChainOSCPad Web Installer

現在の公開版は`1.2.1`です。

- Version 1.2.1: Legacy Encoder設定とv2移行候補の表示を改善
- Version 1.2.0: XIAO D10のStatus / Activity LEDでネットワーク状態とOSC送信処理を表示
- Version 1.1.1: Device Header表記とKey／Encoderのレスポンシブレイアウトを改善
- Version 1.1.0: Device Preset v2 Encoder、明示的なLegacy→v2移行、Key v1適合性、WebUI改善
- Version 1.0.3: Device Preset v2 Encoder対応とEncoder WebUIのレイアウト調整
- Version 1.0.0: AP Modeのキャプティブポータルからの全設定削除に対応した最初の正式版
- Version 0.10.0: Web UIから全設定を削除して再起動する機能を追加

GitHub Pagesでは、公開済みの同一バージョンのGitHub ReleaseからXIAO ESP32S3／ESP32C3／ESP32C5／ESP32C6用ファームウェアを取得します。
