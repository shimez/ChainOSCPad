...
void addButton(Bytes& out,const ButtonInputSetting& button){addU8(out,button.mode);addU8(out,button.pressMessageCount);addU8(out,button.releaseMessageCount);for(uint8_t i=0;i<button.pressMessageCount;++i)addMessage(out,button.pressMessages[i]);for(uint8_t i=0;i<button.releaseMessageCount;++i)addMessage(out,button.releaseMessages[i]);addString(out,button.sequence.address);addU8(out,button.sequence.type);addFloat(out,button.sequence.start);addFloat(out,button.sequence.end);addFloat(out,button.sequence.step);addU8(out,static_cast<uint8_t>(button.sequence.progressionMode));}
bool takeButton(const Bytes& in,size_t& at,ButtonInputSetting& button,bool encoder=false,bool hasProgression=false){uint8_t mode,pressCount,releaseCount,sequenceType;if(!takeU8(in,at,mode)||mode>(encoder?INPUT_MODE_ROTATION_RESET:INPUT_MODE_SEQUENCE)||!takeU8(in,at,pressCount)||!takeU8(in,at,releaseCount)||pressCount>MAX_OSC_MESSAGES||releaseCount>MAX_OSC_MESSAGES||pressCount+releaseCount>MAX_OSC_MESSAGES)return false;button.mode=static_cast<InputMode>(mode);button.pressMessageCount=pressCount;button.releaseMessageCount=releaseCount;for(uint8_t i=0;i<pressCount;++i)if(!takeMessage(in,at,button.pressMessages[i]))return false;for(uint8_t i=0;i<releaseCount;++i)if(!takeMessage(in,at,button.releaseMessages[i]))return false;if(!takeString(in,at,button.sequence.address)||!takeU8(in,at,sequenceType)||!takeFloat(in,at,button.sequence.start)||!takeFloat(in,at,button.sequence.end)||!takeFloat(in,at,button.sequence.step))return false;uint8_t progression=0;if(hasProgression&&(!takeU8(in,at,progression)||progression>1))return false;button.sequence.progressionMode=static_cast<SequenceProgressionMode>(progression);button.sequence.type=static_cast<OscValueType>(sequenceType);button.sequence.current=button.sequence.start;button.sequence.direction=SequenceDirection::Forward;return encoder||inputButtonSettingValid(button);}
Bytes encodeKey(const KeyInputSetting& setting){Bytes out;out.reserve(256);addU8(out,4);addString(out,setting.displayName);addButton(out,setting.button);return out;}
bool decodeKey(const Bytes& in,KeyInputSetting& setting){size_t at=0;uint8_t version;if(!takeU8(in,at,version)||(version!=3&&version!=4)||!takeString(in,at,setting.displayName)||!takeButton(in,at,setting.button,false,version==4)||at!=in.size())return false;return !setting.displayName.isEmpty()&&setting.displayName.length()<=64;}
Bytes encodeEncoder(const EncoderInputSetting& setting){
  Bytes out;out.reserve(384);addU8(out,8);addU8(out,setting.model);addString(out,setting.displayName);addString(out,setting.rotationAddress);
  if(setting.model==ENCODER_SETTINGS_LEGACY){const LegacyEncoderRotationSetting& legacy=setting.legacy;addU8(out,legacy.sendIncrement?1:0);addU8(out,legacy.wrapAround?1:0);addFloat(out,legacy.absoluteInputMin);addFloat(out,legacy.absoluteInputMax);addFloat(out,legacy.incrementScale);addFloat(out,legacy.outputMin);addFloat(out,legacy.outputMax);addU8(out,legacy.outputType);}
  else{addU8(out,setting.rotationMode);addU16(out,setting.rangeSteps);addU8(out,setting.wrapAround?1:0);addU8(out,setting.clockwiseIncreases?1:0);addFloat(out,setting.outputMin);addFloat(out,setting.outputMax);addU8(out,setting.outputType);addString(out,setting.clockwiseValue);addString(out,setting.counterClockwiseValue);}
  addButton(out,setting.push);if(setting.model==ENCODER_SETTINGS_V2&&setting.push.mode==INPUT_MODE_ROTATION_RESET)addString(out,setting.resetValue);return out;
}
bool decodeEncoder(const Bytes& in,EncoderInputSetting& setting){
  size_t at=0;uint8_t version;if(!takeU8(in,at,version))return false;
   if(version==8||version==7||version==6){
    uint8_t model;if(!takeU8(in,at,model)||model>ENCODER_SETTINGS_V2||!takeString(in,at,setting.displayName)||!takeString(in,at,setting.rotationAddress))return false;setting.model=static_cast<EncoderSettingsModel>(model);
    if(setting.model==ENCODER_SETTINGS_LEGACY){uint8_t increment,wrap,type;LegacyEncoderRotationSetting& legacy=setting.legacy;if(!takeU8(in,at,increment)||!takeU8(in,at,wrap)||!takeFloat(in,at,legacy.absoluteInputMin)||!takeFloat(in,at,legacy.absoluteInputMax)||!takeFloat(in,at,legacy.incrementScale)||!takeFloat(in,at,legacy.outputMin)||!takeFloat(in,at,legacy.outputMax)||!takeU8(in,at,type))return false;legacy.sendIncrement=increment!=0;legacy.wrapAround=wrap!=0;legacy.outputType=static_cast<OscValueType>(type);}
    else{uint8_t mode,type,wrap,clockwiseIncreases;if(!takeU8(in,at,mode)||!takeU16(in,at,setting.rangeSteps)||!takeU8(in,at,wrap)||!takeU8(in,at,clockwiseIncreases)||!takeFloat(in,at,setting.outputMin)||!takeFloat(in,at,setting.outputMax)||!takeU8(in,at,type)||!takeString(in,at,setting.clockwiseValue)||!takeString(in,at,setting.counterClockwiseValue))return false;setting.rotationMode=static_cast<EncoderRotationMode>(mode);setting.wrapAround=wrap!=0;setting.clockwiseIncreases=clockwiseIncreases!=0;setting.outputType=static_cast<OscValueType>(type);}
     if(!takeButton(in,at,setting.push,true,version==8))return false;
     if(version>=7&&setting.model==ENCODER_SETTINGS_V2&&setting.push.mode==INPUT_MODE_ROTATION_RESET){if(!takeString(in,at,setting.resetValue))return false;setting.resetValueConfigured=true;}
    if(at!=in.size())return false;
  }
  else if(version==5){uint8_t mode,type,wrap,clockwiseIncreases;setting.model=ENCODER_SETTINGS_V2;if(!takeString(in,at,setting.displayName)||!takeString(in,at,setting.rotationAddress)||!takeU8(in,at,mode)||!takeU16(in,at,setting.rangeSteps)||!takeU8(in,at,wrap)||!takeU8(in,at,clockwiseIncreases)||!takeFloat(in,at,setting.outputMin)||!takeFloat(in,at,setting.outputMax)||!takeU8(in,at,type)||!takeString(in,at,setting.clockwiseValue)||!takeString(in,at,setting.counterClockwiseValue)||!takeButton(in,at,setting.push)||at!=in.size())return false;setting.rotationMode=static_cast<EncoderRotationMode>(mode);setting.wrapAround=wrap!=0;setting.clockwiseIncreases=clockwiseIncreases!=0;setting.outputType=static_cast<OscValueType>(type);}
  else if(version==3||version==4){uint8_t increment,type,wrap=1;LegacyEncoderRotationSetting& legacy=setting.legacy;setting.model=ENCODER_SETTINGS_LEGACY;if(!takeString(in,at,setting.displayName)||!takeString(in,at,setting.rotationAddress)||!takeU8(in,at,increment)||(version>=4&&!takeU8(in,at,wrap))||!takeFloat(in,at,legacy.absoluteInputMin)||!takeFloat(in,at,legacy.absoluteInputMax)||!takeFloat(in,at,legacy.incrementScale)||!takeFloat(in,at,legacy.outputMin)||!takeFloat(in,at,legacy.outputMax)||!takeU8(in,at,type)||!takeButton(in,at,setting.push)||at!=in.size())return false;legacy.sendIncrement=increment!=0;legacy.wrapAround=wrap!=0;legacy.outputType=static_cast<OscValueType>(type);}
  else return false;
...
bool inputSequenceSettingValid(const SequenceSetting& s){return inputOscAddressValid(s.address)&&typeValid(s.type)&&static_cast<uint8_t>(s.progressionMode)<=1&&isfinite(s.start)&&isfinite(s.end)&&isfinite(s.step)&&s.step!=0.0f&&!(s.start<s.end&&s.step<0.0f)&&!(s.start>s.end&&s.step>0.0f);}
...
bool inputEncoderRotationResetValueValid(const EncoderInputSetting& s){if(s.push.mode!=INPUT_MODE_ROTATION_RESET)return true;if(!s.resetValueConfigured)return false;if(s.rotationMode==ENCODER_ROTATION_AMOUNT){float value;if(!inputParseFloat(s.resetValue,value)||value<s.outputMin||value>s.outputMax)return false;if(s.outputType==OSC_TYPE_INT){const double rounded=round(static_cast<double>(value));if(rounded<INT32_MIN||rounded>INT32_MAX)return false;}return true;}if(s.outputType==OSC_TYPE_STRING)return s.resetValue.length()<=128;if(s.outputType==OSC_TYPE_FLOAT){float value;return inputParseFloat(s.resetValue,value);}int32_t value;return inputParseInt(s.resetValue,value);}
bool inputEncoderSettingValid(const EncoderInputSetting& s){
  if(!inputOscAddressValid(s.rotationAddress)||s.push.mode>INPUT_MODE_ROTATION_RESET||s.push.pressMessageCount+s.push.releaseMessageCount>8||!inputSequenceSettingValid(s.push.sequence))return false;
  for(uint8_t i=0;i<s.push.pressMessageCount;++i)if(!inputOscMessageValid(s.push.pressMessages[i]))return false;for(uint8_t i=0;i<s.push.releaseMessageCount;++i)if(!inputOscMessageValid(s.push.releaseMessages[i]))return false;
  if(s.model==ENCODER_SETTINGS_LEGACY){if(s.push.mode>INPUT_MODE_SEQUENCE)return false;const LegacyEncoderRotationSetting& legacy=s.legacy;return typeValid(legacy.outputType)&&isfinite(legacy.absoluteInputMin)&&isfinite(legacy.absoluteInputMax)&&legacy.absoluteInputMax>legacy.absoluteInputMin&&isfinite(legacy.incrementScale)&&isfinite(legacy.outputMin)&&isfinite(legacy.outputMax);}
  if(s.model!=ENCODER_SETTINGS_V2||!typeValid(s.outputType))return false;
  if(!inputEncoderRotationResetValueValid(s))return false;if(s.rotationMode==ENCODER_ROTATION_AMOUNT)return s.rangeSteps>=1&&amountOutputValid(s.outputMin,s.outputMax,s.outputType);if(s.rotationMode!=ENCODER_ROTATION_DIRECTION||s.clockwiseValue.length()>128||s.counterClockwiseValue.length()>128)return false;if(s.outputType==OSC_TYPE_STRING)return true;if(s.outputType==OSC_TYPE_FLOAT){float clockwise,counterClockwise;return inputParseFloat(s.clockwiseValue,clockwise)&&inputParseFloat(s.counterClockwiseValue,counterClockwise);}int32_t clockwise,counterClockwise;return inputParseInt(s.clockwiseValue,clockwise)&&inputParseInt(s.counterClockwiseValue,counterClockwise);
}
bool encoderMeaningEqual(const EncoderInputSetting& a,const EncoderInputSetting& b){if(a.model!=b.model)return false;if(a.model==ENCODER_SETTINGS_LEGACY){const LegacyEncoderRotationSetting& x=a.legacy;const LegacyEncoderRotationSetting& y=b.legacy;return x.sendIncrement==y.sendIncrement&&x.wrapAround==y.wrapAround&&x.absoluteInputMin==y.absoluteInputMin&&x.absoluteInputMax==y.absoluteInputMax&&x.incrementScale==y.incrementScale&&x.outputMin==y.outputMin&&x.outputMax==y.outputMax&&x.outputType==y.outputType;}if(a.rotationMode!=b.rotationMode)return false;if(a.rotationMode!=ENCODER_ROTATION_AMOUNT)return true;return a.rangeSteps==b.rangeSteps&&a.wrapAround==b.wrapAround&&a.clockwiseIncreases==b.clockwiseIncreases&&a.outputMin==b.outputMin&&a.outputMax==b.outputMax&&a.outputType==b.outputType;}
