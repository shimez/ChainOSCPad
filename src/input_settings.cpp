...
Bytes encodeEncoder(const EncoderInputSetting& setting){
  Bytes out;out.reserve(352);addU8(out,6);addU8(out,setting.model);addString(out,setting.displayName);addString(out,setting.rotationAddress);
  ...
  addButton(out,setting.push);return out;
}
bool decodeEncoder(const Bytes& in,EncoderInputSetting& setting){
  ...
  if(version==6){
    ...
    if(!takeButton(in,at,setting.push)||at!=in.size())return false;
  }
