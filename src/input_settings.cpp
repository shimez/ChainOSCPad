...
Bytes encodeEncoder(const EncoderInputSetting& setting){Bytes out;out.reserve(320);addU8(out,4);addString(out,setting.displayName);addString(out,setting.rotationAddress);addU8(out,setting.sendIncrement?1:0);addU8(out,setting.wrapAround?1:0);addFloat(out,setting.absoluteInputMin);addFloat(out,setting.absoluteInputMax);addFloat(out,setting.incrementScale);addFloat(out,setting.outputMin);addFloat(out,setting.outputMax);addU8(out,setting.outputType);addButton(out,setting.click);return out;}
bool decodeEncoder(const Bytes& in,EncoderInputSetting& setting){size_t at=0;uint8_t version,increment,type,wrap=1;if(!takeU8(in,at,version)||(version!=3&&version!=4)||!takeString(in,at,setting.displayName)||!takeString(in,at,setting.rotationAddress)||!takeU8(in,at,increment)||(version>=4&&!takeU8(in,at,wrap))||!takeFloat(in,at,setting.absoluteInputMin)||!takeFloat(in,at,setting.absoluteInputMax)||!takeFloat(in,at,setting.incrementScale)||!takeFloat(in,at,setting.outputMin)||!takeFloat(in,at,setting.outputMax)||!takeU8(in,at,type)||!takeButton(in,at,setting.click)||at!=in.size())return false;setting.sendIncrement=increment!=0;setting.wrapAround=wrap!=0;setting.outputType=static_cast<OscValueType>(type);return !setting.displayName.isEmpty()&&setting.displayName.length()<=64&&inputEncoderSettingValid(setting);}
