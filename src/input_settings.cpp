...
void setDefaults(){
  for(uint8_t i=0;i<KEY_COUNT;++i){keySettings[i].displayName=String("Key ")+(i+1);defaultButton(keySettings[i].button,String("/chainoscpad/key/")+(i+1),String("/chainoscpad/key/")+(i+1)+"/sequence");}
  encoderSetting=EncoderInputSetting();
  defaultButton(encoderSetting.click,"/avatar/parameters/EncoderClick","/avatar/parameters/EncoderSeq");
}
