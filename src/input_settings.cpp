...
Bytes encodeEncoder(const EncoderInputSetting& setting){
  Bytes out;out.reserve(384);addU8(out,7);addU8(out,setting.model);addString(out,setting.displayName);addString(out,setting.rotationAddress);
...
bool decodeEncoder(const Bytes& in,EncoderInputSetting& setting){
  ...
  if(version==7||version==6){
    ...
    if(!takeButton(in,at,setting.push,true))return false;
