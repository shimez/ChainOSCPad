...
.validation-error{display:block;margin-top:5px;color:#b91c1c;font-weight:400;text-align:left}
</style><script>
...
function toggleMode(s,g){document.getElementById('pr-'+g).style.display=s.value==='0'?'block':'none';document.getElementById('seq-'+g).style.display=s.value==='1'?'block':'none';let reset=document.getElementById('reset-'+g);if(reset)reset.style.display=s.value==='2'?'block':'none'}
...
function validateEncoderResetValue(form){let input=form.querySelector('[name=enc_reset_value]'),push=form.querySelector('[name=enc_mode]');if(!input||!push||push.value!=='2'||input.offsetParent===null)return true;let mode=form.querySelector('[name=enc_emode]').value,type=form.querySelector('[name=enc_eotype]').value,value=input.value.trim(),message='';if(utf8Bytes(input.value)>128)message=tx('Keep the value within 128 bytes in UTF-8','値はUTF-8で128バイト以内にしてください');else if(mode==='amount'){let n=Number(value),f=Math.fround(n),min=Number(form.querySelector('[name=enc_eomin]').value),max=Number(form.querySelector('[name=enc_eomax]').value);if(!value||!Number.isFinite(n)||!Number.isFinite(f)||(n!==0&&f===0))message=tx('Enter a finite OSC float32 value','有限のOSC float32値を入力してください');else if(n<min||n>max)message=tx('Reset Value must be between Min and Max.','リセット値は最小値から最大値の範囲内で指定してください。');else if(type==='1'){let rounded=Math.round(Math.abs(f))*Math.sign(f);if(rounded<-2147483648||rounded>2147483647)message=tx('Converted Int must fit in OSC int32','変換後のInt値はOSC int32の範囲内にしてください')}}else if(type==='0'&&float32Invalid(value))message=tx('Enter a finite OSC float32 value','有限のOSC float32値を入力してください');else if(type==='1'){if(!/^[+-]?\d+$/.test(value))message=tx('Enter a decimal OSC int32','OSC int32の10進整数を入力してください');else{let n=BigInt(value);if(n<-2147483648n||n>2147483647n)message=tx('Int must be between -2147483648 and 2147483647','Intは-2147483648～2147483647の範囲で入力してください')}}return validationError(input,message)}
function validateForm(form){let valid=true;form.querySelectorAll('.addr,.seq-address input,[name=enc_erot],.osc-row input:not(.addr),[name=enc_range_steps],[name=enc_eomin],[name=enc_eomax],[name=enc_clockwise],[name=enc_counter_clockwise],[name$=seq_start],[name$=seq_end],[name$=seq_step]').forEach(input=>{if(input.offsetParent!==null&&!validateInput(input))valid=false});if(!validateEncoderResetValue(form))valid=false;if(!valid){let first=form.querySelector('.invalid');if(first){first.focus();first.scrollIntoView({behavior:'smooth',block:'center'})}alert(tx('Please correct the settings highlighted in red.','赤く表示された設定項目を修正してください。'))}return valid}
...
function encMode(s){let rotation=s.closest('.rotation'),amount=s.value==='amount',grid=rotation.querySelector('.encoder-rotation-grid');rotation.querySelectorAll('.amount-mode').forEach(x=>{x.hidden=!amount;x.style.display=amount?'':'none'});rotation.querySelectorAll('.direction-mode').forEach(x=>{x.hidden=amount;x.style.display=amount?'none':''});if(grid){grid.classList.toggle('amount-layout',amount);grid.classList.toggle('direction-layout',!amount)}}
function legacyEncMode(s){let grid=s.closest('.legacy-grid'),absolute=s.value==='amount';if(grid)grid.querySelectorAll('.legacy-absolute-setting').forEach(x=>x.classList.toggle('legacy-mode-hidden',!absolute))}
...
function selectInputGuideCard(card){if(!card)return;inputGuideExclusive=true;showInputGuideIndexes([Number(card.dataset.guideIndex)])}
...
#undef head
  ...
  String buttonEditor(const String &g, const String &prefix, const ButtonInputSetting &b, const char *modeLabel, const char *seqTitle, bool encoder = false, const String &resetValue = "0.5")
  {
    const bool full = b.pressMessageCount + b.releaseMessageCount >= 8;
    String h = "<div class='mode'><label>" + String(modeLabel) + "<select name='" + prefix + "mode' onchange=\"toggleMode(this,'" + g + "')\"><option value='0'" + (b.mode == INPUT_MODE_PRESS_RELEASE ? " selected" : "") + ">" + tr("Press / Release", "押した時／離した時") + "</option><option value='1'" + (b.mode == INPUT_MODE_SEQUENCE ? " selected" : "") + ">" + tr("Sequence", "シーケンス") + "</option>" + (encoder ? "<option value='2'" + String(b.mode == INPUT_MODE_ROTATION_RESET ? " selected" : "") + ">" + tr("Rotation Reset", "リセット") + "</option>" : String()) + "</select></label></div>";
    h += "<div id='pr-" + g + "' class='pr' data-prefix='" + prefix + "' style='display:" + String(b.mode == INPUT_MODE_PRESS_RELEASE ? "block" : "none") + "'><div class='usage'><b>" + tr("Messages ", "メッセージ ") + "<span class='total'>" + String(b.pressMessageCount + b.releaseMessageCount) + "</span> / 8</b><span>" + tr("Press + Release", "押した時＋離した時") + "</span></div><input class='pc' type='hidden' name='" + prefix + "p_count' value='" + String(b.pressMessageCount) + "'><input class='rc' type='hidden' name='" + prefix + "r_count' value='" + String(b.releaseMessageCount) + "'><div class='tabs'><button type='button' class='active' onclick=\"showEvent(this,'press')\">" + tr("Press", "押した時") + "</button><button type='button' onclick=\"showEvent(this,'release')\">" + tr("Release", "離した時") + "</button></div><div class='panel press'><div class='list'>";
    for (uint8_t i = 0; i < b.pressMessageCount; ++i)
      h += row(prefix, "press", i, b.pressMessages[i]);
  ...
  bool parseButton(const String &prefix, ButtonInputSetting &b, bool encoder = false)
  {
    const String modeText = server.arg(prefix + "mode");
    if (modeText != "0" && modeText != "1" && (!encoder || modeText != "2")) return false;
    b.mode = static_cast<InputMode>(modeText.toInt());
  ...
  bool parseEncoder(EncoderInputSetting &e, bool &migrationSave)
  {
    ...
    if (editModel == "legacy")
    {
      ...
      e.legacy.wrapAround = server.hasArg(prefix + "legacy_wrap");
    ...
    if (!parseButton(prefix, e.push, true)) return false;
    if (e.push.mode == INPUT_MODE_ROTATION_RESET) { e.resetValue = server.arg(prefix + "reset_value"); e.resetValue.trim(); e.resetValueConfigured = true; }
    return !e.displayName.isEmpty() && e.displayName.length() <= 64 && inputEncoderSettingValid(e);
  }
...
document.addEventListener('DOMContentLoaded',()=>{['forget-wifi-form','reset-settings-form'].forEach(id=>{const form=document.getElementById(id);if(form)form.addEventListener('submit',submitSystemForm)})});
  ...
  String inputsPrefix(const String &msg = "", bool err = false)
  {
    ...
    h += "<style>.legacy-badge{background:#fff1d6;color:#8a5200}.v2-badge{background:#dff5e6;color:#176b39}.candidate-badge{background:#e7e2ff;color:#5135a8}#encoder-body>.key-grid+.rotation{margin-top:22px}.rotation,.push{margin-top:14px;padding:0 0 0 12px;border:0;border-radius:0;background:transparent}.rotation{border-left:5px solid #fd7e14}.push{border-left:5px solid #20c997}.rotation>h3,.push>h3{margin:0 0 14px}.legacy-status,.migration-notice{margin:14px 0 18px;padding:14px 16px;border:1px solid #f0c36a;border-radius:9px;background:#fff9e8}.legacy-status{display:flex;flex-direction:column;gap:5px}.legacy-status span{color:#68758a}.legacy-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.legacy-grid .full{grid-column:1/-1}.legacy-grid .legacy-mode-hidden{visibility:hidden;pointer-events:none}.legacy-wrap label{display:flex;align-items:center;gap:6px;margin:0}.legacy-wrap input{width:auto;margin:0}.migration-action{margin-top:14px;padding-top:14px}.migration-action button,.cancel-migration{margin:0;padding:11px 16px;border:0;border-radius:7px;background:#3267e3;color:#fff;font:inherit}.migration-notice ul{margin:10px 0 12px;padding-left:22px}.migration-confirm{display:flex!important;flex-direction:row!important;align-items:center;gap:9px}.migration-confirm input{width:auto!important;margin:0}.cancel-migration{margin-top:12px;background:#64748b}@media(max-width:800px){.legacy-grid{grid-template-columns:1fr}.legacy-grid .full{grid-column:auto}.legacy-grid .legacy-mode-hidden{display:none}}</style>";
    h += "<style>.legacy-mode{order:1}.legacy-input-min{order:2}.legacy-input-max{order:3}.legacy-wrap{order:4}.legacy-scale{order:5}.legacy-output-min{order:6}.legacy-output-max{order:7}.legacy-type{order:8}.encoder-rotation-grid>.hidden{display:none!important}</style>";
    h += "<style>body{padding:24px clamp(16px,4vw,64px)}main{max-width:1280px}.overview-card,.settings-tools{border-left:0}.device{min-width:0;max-width:100%}.device-head{min-width:0;gap:12px}.device-title{min-width:0;flex:1;flex-wrap:wrap}.device-title>span:last-child{min-width:0;overflow-wrap:anywhere}.device-menu-wrap{flex:0 0 auto}.device-body,.key-grid,.pr,.panel,.list{min-width:0}.key-settings{grid-template-columns:repeat(2,minmax(0,1fr));align-items:start}.key-settings>.mode{min-width:0}.card-heading-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.card-heading-row h2{margin:0}.language-card select{width:180px;margin:0}.system-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px}.system-item{display:flex;flex-direction:column;gap:7px;padding:13px 15px;border:1px solid #dce2ea;border-radius:8px;background:#f8fafc}.system-item code,.meta code{overflow-wrap:anywhere}.osc-grid{display:grid;grid-template-columns:2fr 1fr;gap:18px}.settings-tools h2{margin:0 0 18px}.settings-tools p{color:#68758a;margin:0 0 18px}.tool-row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.tool-row a,.tool-row button{display:block;width:100%;margin:0;padding:15px;border:0;border-radius:7px;background:#3267e3;color:#fff;text-align:center;text-decoration:none;font-size:16px}.sequence{background:#fbfcfe;border:1px solid #d9e0e9;border-left:1px solid #d9e0e9;border-radius:11px;padding:18px;margin-top:14px}.sequence p{color:#68758a}.seqgrid{grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.seqgrid .seq-address{grid-column:1/-1}.import-status{min-height:20px;margin:9px 0 0;color:#526075}.device-menu-wrap{position:relative}.device-menu{position:absolute;z-index:20;right:0;top:44px;width:310px;max-width:calc(100vw - 32px);padding:8px;border:1px solid #dce2ea;border-radius:9px;background:#fff;box-shadow:0 8px 24px #0003}.device-menu[hidden]{display:none}.device-menu a,.device-menu button{display:block;width:100%;margin:0;padding:12px 15px;border:0;border-radius:6px;background:#fff;color:#253047;text-align:left;text-decoration:none;font-size:16px;white-space:nowrap}.device-menu a:hover,.device-menu button:hover{background:#f1f4f8}@media(max-width:900px){.system-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.seqgrid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:800px){.key-settings{grid-template-columns:1fr}}@media(max-width:600px){body{padding:12px}.tool-row,.osc-grid,.system-grid,.seqgrid{grid-template-columns:1fr}.seqgrid .seq-address{grid-column:auto}.device-menu{width:285px}.card-heading-row{align-items:flex-start}.language-card select{width:140px}}</style><header><h1>ChainOSCPad Settings</h1>" + (msg.isEmpty() ? "" : String("<p class='") + (err ? "err" : "ok") + "'>" + esc(msg) + "</p>") + "</header>" + languageCard() + systemCard() + wifiStatusCard() + settingsBackupCard() + oscSettingsCard() + "<h2 style='margin:28px 4px 14px'>" + tr("Input Settings", "入力設定") + "</h2><form id='input-form' method=post action='/inputs/save-all'>";
    return h;
  }
  ...
  String legacyEncoderCard(const EncoderInputSetting &e)
  {
    ...
    h.replace("<label>" + String(tr("Legacy Mode", "旧形式モード")) + "<select name=enc_legacy_mode>", "<label class=legacy-mode>" + String(tr("Mode", "モード")) + "<select name=enc_legacy_mode onchange=legacyEncMode(this)>");
    const String oldWrap = "<label>" + String(tr("At Minimum / Maximum", "最小値・最大値の先")) + "<select name=enc_legacy_wrap><option value=wrap" + String(x.wrapAround ? " selected" : "") + ">" + tr("Wrap (Legacy semantics)", "ループする（旧形式の動作）") + "</option><option value=stop" + String(x.wrapAround ? "" : " selected") + ">" + tr("Stop", "停止する") + "</option></select></label>";
    const String hidden = x.sendIncrement ? " legacy-mode-hidden" : "";
    h.replace(oldWrap, "<div class='legacy-wrap legacy-absolute-setting" + hidden + "'><label><input type=checkbox name=enc_legacy_wrap value=wrap" + String(x.wrapAround ? " checked" : "") + "> " + tr("Wrap around", "範囲をループする") + "</label></div>");
    h.replace("<label>" + String(tr("Type", "型")) + "<select name='enc_legacy_type'>", "<label class=legacy-type>" + String(tr("Type", "型")) + "<select name='enc_legacy_type'>");
    h.replace("<label>" + String(tr("Absolute Input Min", "絶対値入力の最小値")), "<label class='legacy-input-min legacy-absolute-setting" + hidden + "'>" + String(tr("Absolute Input Min", "絶対値入力の最小値")));
    h.replace("<label>" + String(tr("Absolute Input Max", "絶対値入力の最大値")), "<label class='legacy-input-max legacy-absolute-setting" + hidden + "'>" + String(tr("Absolute Input Max", "絶対値入力の最大値")));
    h.replace("<label>" + String(tr("Increment Scale", "増分倍率")), "<label class=legacy-scale>" + String(tr("Increment Scale", "増分倍率")));
    h.replace("<label>" + String(tr("Output Min", "出力最小値")), "<label class=legacy-output-min>" + String(tr("Output Min", "出力最小値")));
    h.replace("<label>" + String(tr("Output Max", "出力最大値")), "<label class=legacy-output-max>" + String(tr("Output Max", "出力最大値")));
    h.replace("<span class='badge badge-type'>Encoder</span>", "<span class='badge badge-type'>#" + String(KEY_COUNT + 1) + " Encoder</span>");
  ...
  String encoderCard()
  {
    ...
    const String resetDisplay = e.resetValueConfigured ? e.resetValue : (amount ? String((e.outputMin + e.outputMax) * 0.5f, 7) : String("0"));
    ...
    h.replace("</option></select></label></div><div id='pr-enc'", "</option><option value='2'" + String(e.push.mode == INPUT_MODE_ROTATION_RESET ? " selected" : "") + ">" + tr("Rotation Reset", "リセット") + "</option></select></label></div><div id='pr-enc'");
    if (e.push.mode == INPUT_MODE_ROTATION_RESET) h.replace("id='pr-enc' class='pr' data-prefix='enc_' style='display:block'", "id='pr-enc' class='pr' data-prefix='enc_' style='display:none'");
    h.replace("</div></div></div></section>", "</div><div id='reset-enc' class='encoder-reset-value' style='display:" + String(e.push.mode == INPUT_MODE_ROTATION_RESET ? "block" : "none") + "'><label>" + tr("Reset Value", "リセット値") + "<input maxlength='128' name='enc_reset_value' value='" + esc(resetDisplay) + "'></label></div></div></div></section>");
    ...
    h.replace("<span class='badge badge-type'>Encoder</span>", "<span class='badge badge-type'>#" + String(KEY_COUNT + 1) + " Encoder</span>" + String(migration ? "<span class='badge candidate-badge'>v2 candidate</span>" : "<span class='badge v2-badge'>v2</span>"));
